...
VS Code stores OAuth tokens in the OS keyring. In headless/container environments, tokens are lost on process exit unless a keyring daemon is running. This container includes `gnome-keyring` packages for token persistence.
...
The [HPC Code Server Manager](https://github.com/drejom/omhq-hpc-code-server-stack) handles keyring initialization in job scripts. See [issue #28](https://github.com/drejom/omhq-hpc-code-server-stack/issues/28) for implementation details.
